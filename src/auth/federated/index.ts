...
const createAuthFederated = (options: Options): VueAuthModule => {
  ...
  return {
    init: async () => {
      const response = await fetch(options.jwtUrl, {
    ...
    login,
    logout: () => (window.location.href = options.logoutUrl),
    attemptLogin: login,
    getAuthorizationHeader: (): Record<string, string> =>
      state ? { Authorization: `Bearer ${state.jwt}` } : {},
    hasCredential: (corpusId) => (state?.credentials || []).includes(corpusId.toUpperCase()),
    getCredentials: () => state?.credentials || [],
    getUsername: () => state!.username,
    isLoggedIn: () => !!state,
  }
}
export default createAuthFederated
