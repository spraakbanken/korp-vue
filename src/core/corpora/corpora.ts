...
function getCorpora(
  ...
  corpusConstraint: (corpus: Corpus) => boolean = () => true,
): string[] {
  const subIds: string[] = folder.subFolders.flatMap((folder) =>
    getCorpora(folder, corpusConstraint),
  )
 ...
 */
export const getAllCorporaInFolders = (
  lastLevel: Record<string, Folder>,
  folderOrCorpus: string,
): string[] => {
...
export function getSizeInfo(corpus: Corpus): CorpusSizeInfo[] {
  const infos: CorpusSizeInfo[] = [
