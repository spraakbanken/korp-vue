...
onmessage = function (e) {
  ...
  const totalsByCorpus = mapValues(
    corpora,
    (data) => [data.sums.absolute, data.sums.relative] as AbsRelSeq,
  )
  const corporaFreqs = mapValues(corpora, (data) =>
    groupBy(data.rows, (item) => simplifyHitString(item)),
  )
